...
class ApiService {
  ...
  static Future<Map?> getOtpDetails(mobile) async {
    Dio dio = Dio();
    log(ApiEndPoint.getOtp);
    try {
      log({
        "mobile": "$mobile",
      }.toString());
  ...
  static Future<Map?> verifyOtpDetails(mobile, otp) async {
    log([mobile, otp].toString());
    Dio dio = Dio();
    log(ApiEndPoint.verifyOtp);
    try {
      Response response = await dio.post(ApiEndPoint.verifyOtp,
